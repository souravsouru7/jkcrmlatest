  ...
  return (
    <div className={pageWrap}>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[380px_1fr]">
        <Section title="Weekly planner" subtitle="Visit load for the next seven days.">
          <div className="grid grid-cols-7 gap-1 overflow-x-auto p-3 sm:gap-2 sm:p-4">
            {Array.from({ length: 7 }).map((_, index) => {
              ...
              return (
                <div key={key} className={`min-w-16 rounded-lg border p-2 text-center sm:p-3 ${index === 0 ? "border-blue-600 bg-blue-50 dark:bg-blue-500/10" : "border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950"}`}>
        ...
        <Section title="Daily visits" subtitle="Map, call, scope, complete, or reschedule.">
          <div className="flex gap-2 overflow-x-auto border-b border-slate-100 p-4 dark:border-slate-800">
            {FILTERS.map((item) => (
              <button key={item} onClick={() => setFilter(item)} className={`shrink-0 rounded-lg border px-4 py-2 text-sm font-semibold capitalize ${filter === item ? "border-blue-600 bg-blue-600 text-white" : "border-slate-200 bg-white text-slate-600 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300"}`}>
                {item}
              </button>
            ))}
          </div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 gap-3 p-4 lg:grid-cols-2">
              {visible.map((visit) => {
                ...
                return (
                  <article key={visit.id} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950">
                    ...
                    <div className="mt-5 flex flex-wrap gap-2">
                      ...
                      {visit.status === "Scheduled" && (
                        <>
                          <button onClick={() => handleStatus(visit.id, "Completed")} className="rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-emerald-700">Complete</button>
                          <button onClick={() => handleStatus(visit.id, "Rescheduled")} className="rounded-lg bg-amber-500 px-4 py-2.5 text-sm font-semibold text-white hover:bg-amber-600">Reschedule</button>
                        </>
                      )}
                    </div>
                  </article>
